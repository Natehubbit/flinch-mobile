...
export const {
  ...
  reducers: {
    getNotifications(
      ...
      {
        payload
      }: PayloadAction<NotificationMessage[]>
    ): NotificationMessage[] {
      return payload
    },
    update(
...
const getDeviceToken = () => async (
  ...
  const {
    id,
    token,
    celebrity: { isCeleb, id: celebId }
  } = getState().user
  if (!token) {
    const tkn = await NotificationService.getToken()
    if (isCeleb) {
      const usr = getState().user
      const updatedCeleb = await CelebService.updateCeleb(
        {
          id: celebId,
          token: tkn
        }
...
const getNotifications = () => async (
  ...
  const id = isUser ? userId : celebrity.id
...
const update = (
  ...
  if (updated) {
    const updatedList = notifications.map((d) => {
      if (d.id === id) {
        return { ...d, ...data }
      }
      return d
    ...
    dispatch(
      actions.getNotifications(updatedList)
    )
  }
}
